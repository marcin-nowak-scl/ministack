...
_PATH_KEY_ROOT = re.compile(r"^/v2/apis/([^/]+)/apikeys/?$")
_PATH_KEY_ITEM = re.compile(r"^/v2/apis/([^/]+)/apikeys/([^/]+)/?$")
...
async def _handle_mgmt(method, path, headers, body, query_params):
    ...
    if (m := _PATH_KEY_ROOT.match(path)):
        api_id = m.group(1)
        if method == "POST":
            return _create_api_key(api_id, body)
        if method == "GET":
            return _list_api_keys(api_id)
        return error_response_json("BadRequestException", f"Method {method} not allowed on {path}", 405)

    if (m := _PATH_KEY_ITEM.match(path)):
        api_id, key_id = m.group(1), m.group(2)
        if method == "DELETE":
            return _delete_api_key(api_id, key_id)
        return error_response_json("BadRequestException", f"Method {method} not allowed on {path}", 405)
...
async def _handle_client_frame(api_id: str, connection_id: str, frame: dict, outbox: asyncio.Queue):
    ...
    if ftype == "subscribe":
        ...
        if lambda_arn:
            stok = _bearer_from_auth_dict(_effective_auth(frame, connection_id))
            if not stok:
                await outbox.put({
                    "type": "subscribe_error",
                    "id": sub_id,
                    "errors": [{"errorType": "UnauthorizedException",
                                "message": "subscribe rejected: no Authorization token"}],
                })
                return
            ok, _r = await asyncio.to_thread(
                _events_authorizer_invoke,
                lambda_arn, "subscribe", str(channel), stok,
            )
            if not ok:
                await outbox.put({
                    "type": "subscribe_error",
                    "id": sub_id,
                    "errors": [{"errorType": "UnauthorizedException",
                                "message": "subscribe rejected: AppSync authorizer denied"}],
                })
                return
        elif _strict_auth_enabled() and not _auth_has_known_api_key(
            api_id, _effective_auth(frame, connection_id),
        ):
            await outbox.put({
                "type": "subscribe_error",
                "id": sub_id,
                "errors": [{"errorType": "UnauthorizedException",
                            "message": "subscribe rejected: no valid x-api-key"}],
            })
            return
        async with _connections_lock:
        ...
        if lambda_arn:
            ...
            if not stok:
                await outbox.put({
                    ...
                    "id": pub_id,
                    "errors": [{"errorType": "UnauthorizedException",
                                "message": "publish rejected: no Authorization token"}],
                })
                return
            ok, _r = await asyncio.to_thread(
                _events_authorizer_invoke,
                lambda_arn, "publish", str(channel), stok,
            )
            if not ok:
                await outbox.put({
                    "type": "publish_error",
                    "id": pub_id,
                    "errors": [{"errorType": "UnauthorizedException",
                                "message": "publish rejected: AppSync authorizer denied"}],
                })
                return
        elif _strict_auth_enabled() and not _auth_has_known_api_key(
            api_id, _effective_auth(frame, connection_id),
            ...
            await outbox.put({
                ...
                "id": pub_id,
                "errors": [{"errorType": "UnauthorizedException",
                            "message": "publish rejected: no valid x-api-key"}],
            })
            return
        successful, failed = await _fanout_publish(api_id, channel, events)
        await outbox.put({
