"""Wildcard DNS resolver for AWS-shaped local hostnames on Docker networks.

The resolver itself is service-agnostic: it answers ``*.<suffix>`` for any
configured local zone with MiniStack's IPv4 on the shared Docker network and
forwards unrelated queries upstream. The initial zones are AppSync Events
HTTP/realtime hostnames from optional ``APPSYNC_EVENTS_HTTP_HOST_TEMPLATE`` /
``APPSYNC_EVENTS_REALTIME_HOST_TEMPLATE`` values, or — when unset — defaults
matching AppSync Events ``CreateApi`` ``dns`` responses:
``appsync-api.{region}.{MINISTACK_HOST}`` and
``appsync-realtime-api.{region}.{MINISTACK_HOST}``.

Enable with ``MINISTACK_DNS_RESOLVER=1``. Port defaults to 53 (``MINISTACK_DNS_PORT``).
"""
...
def resolver_enabled() -> bool:
    v = os.environ.get("MINISTACK_DNS_RESOLVER", "0").strip().lower()
...
def _default_zone_suffixes() -> list[str]:
    """Initial local zones: default AppSync Events ``dns`` suffixes."""
    region = get_region()
...
def zone_suffixes() -> list[str]:
    """Host suffixes for wildcard answers.

    Keep this as a single list-builder so future AWS-shaped local service
    hostnames can share the same resolver without changing the UDP protocol
    handling below.
    """
    out: list[str] = []
    for key in (
...
def _get_ministack_network(client) -> str | None:
    ...
    try:
        ...
        logger.debug("dns_resolver: could not inspect MiniStack container", exc_info=True)
        return None


def _get_ministack_container_ipv4_on_network(
    client, network_name: str | None = None
) -> str | None:
    hostname = os.environ.get("HOSTNAME", "").strip()
    if not hostname:
        return None
    network = network_name if network_name is not None else _get_ministack_network(client)
    if not network:
        return None
    try:
        ...
        logger.debug("dns_resolver: could not read MiniStack IPv4 on %s", network, exc_info=True)
        return None
...
def resolve_ministack_dns_answer_ip() -> bytes | None:
    ...
    ip = _get_ministack_container_ipv4_on_network(dc)
    if ip:
